...
import static Panel.DevisCreatePanel.calculateUnitPriceFenetreBattante;
//import static Panel.DevisCreatePanel.calculateUnitPriceFenetreBattante;
import static Panel.DevisCreatePanel.calculateUnitPriceFenetreCoulissante;
...
import static Panel.DevisCreatePanel.parseProfileAluString;
import static Panel.DevisCreatePanel.parseTypeVitreString;
import static Panel.DevisCreatePanel.prixChassisFixeFenetre;
import static Panel.DevisCreatePanel.prixChassisOuvrantFenetre;
import static Panel.DevisCreatePanel.prixJointFenetre;
import static Panel.DevisCreatePanel.prixPaumelleFenetre;
import static Panel.DevisCreatePanel.prixPoigneeFenetre;
import static Panel.DevisCreatePanel.prixRouletteFenetre;
import static Panel.DevisCreatePanel.prixSerrureFenetre;
import static Panel.DevisCreatePanel.prixVisAssemblageFenetre;
import static Panel.DevisCreatePanel.prixVitreFenetre;
import static Panel.DevisCreatePanel.produitId;
import static Panel.DevisCreatePanel.quantiteBarreChassisFixeFenetre;
import static Panel.DevisCreatePanel.quantiteBarreChassisOuvrantFenetre;
import java.math.BigDecimal;
import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.atomic.AtomicInteger;

public class EditFenetreForm extends JDialog {
    // Fenêtre
    ...
    private JPanel SpinnerContainerPanelFenetre;
    private JScrollPane scrollPaneSousligneFenetre;
    private JButton addSousLigneFenetreButton;

    private final ArrayList<JButton> removeButtonsFenetre = new ArrayList<>();
...
//    private int profileAluIdtypeOuvrant;
    ...
    private Integer nbFenetreRoulette;
    ...
    public EditFenetreForm(Component owner, String designation, List<Object[]> subRows, int selectedRow) throws Exception {

        super(SwingUtilities.getWindowAncestor(owner), "Modifier la Fenetre", Dialog.ModalityType.APPLICATION_MODAL);
        setLayout(new BorderLayout());
        JPanel FenetrePanel = createFenetrePanel();

        fillFenetreComboBoxes( selectedRow, subRows);
        fillSpinnersAndQuantity(subRows);

        add(FenetrePanel, BorderLayout.CENTER);
    ...
    private JPanel createSpinnerFenetrePanel() {
        ...
        scrollPaneSousligneFenetre.setPreferredSize(new Dimension(260, 236));
        ...
        addSousLigneFenetreButton = new JButton();
        addSousLigneFenetreButton.setIcon(new ImageIcon(getClass().getResource("/Icon/icons8-add-16.png")));
        addSousLigneFenetreButton.setHorizontalTextPosition(SwingConstants.CENTER);

        addSousLigneFenetreButton.addActionListener((ActionEvent e) -> {
  ...
  Dimension buttonSize = new Dimension(16, 16);
       ...
       addSousLigneFenetreButton.setPreferredSize(buttonSize);

        buttonsPanel.add(addSousLigneFenetreButton, BorderLayout.EAST);
    ...
    private JPanel createSousLigneFenetrePanel() {
        ...
        largeurFenetreSpinner = DevisCreatePanel.configureSpinnerBigDecimal();
        hauteurFenetreSpinner = DevisCreatePanel.configureSpinnerBigDecimal();
        quantiteFenetreSpinner = DevisCreatePanel.configureSpinnerInteger();

        DevisCreatePanel.addField(createsousLignePanel, "Largeur(mm):", largeurFenetreSpinner);
        DevisCreatePanel.addField(createsousLignePanel, "Hauteur(mm):", hauteurFenetreSpinner);
        DevisCreatePanel.addField(createsousLignePanel, "Quantité:", quantiteFenetreSpinner);
   ...
   if (chassisTypeFixeId == -1) {
            ...
            String newDesignation = buildFenetreDesignation();  // ou buildFenetreDesignation(), etc.
            ...
            List<List<JSpinner>> panelSpinners = DevisCreatePanel.getSpinnerValues(SpinnerContainerPanelFenetre);
    ...
    return; // ou lancez une exception, ou affichez un message d'erreur
     ...
     System.out.println("Largeur "+largeur+" hauteur "+ hauteur);
                ...
                if (typeOuvertureId == 1 ) {
                      ...
                      nbPoignee = null;
                       nbPaumelle = null;
                        unitePrice = calculateUnitPriceFenetreCoulissante(hauteur, largeur, nbVentaux, prixUnitaireChassisFixe,longueurChassisFixe,prixUnitaireChassisOuvrant,longueurChassisOuvrant,prixUnitaireVitre,prixUnitaireJoint,prixUnitaireVis,prixUnitaireRoulette,prixUnitaireSerrure);

           
        } else if (typeOuvertureId == 2||typeOuvertureId == 3) { // Pour une fenêtre battante 
                nbRoulette = null;
       unitePrice = calculateUnitPriceFenetreBattante(hauteur, largeur, nbVentaux, prixUnitaireChassisFixe,longueurChassisFixe,prixUnitaireChassisOuvrant,longueurChassisOuvrant,prixUnitaireVitre,prixUnitaireJoint,prixUnitaireVis,prixUnitairePaumelle,prixUnitaireSerrure,prixUnitairePoignee);
                   ...
                   String dimensions = largeur + " mm (Lt) x " + hauteur + " mm (Ht)";
                    ...
                    // Ajouter les nouvelles sous-lignes mises à jour
                     updatedSubRows.add(new Object[]{"", chassisTypeFixeId, chassisTypeOuvrantId, nbVentaux, vitrageId, typeVitreId, structureId,paumelleId ,rouletteId,  serrureId,poigneeId, visId ,jointId,dimensions, unitePrice, quantite, totalPrice,nbRoulette ,
    nbPaumelle,
      nbPoignee,
      nbVisAssemblage ,
        nbSerrures ,
 longueurTotalJoint ,
  quantiteBarreChassisFixeFenetre,quantiteBarreChassisOuvrantFenetre,prixChassisFixeFenetre,prixChassisOuvrantFenetre,prixVitreFenetre,prixJointFenetre,prixRouletteFenetre,prixPaumelleFenetre,prixSerrureFenetre,prixPoigneeFenetre,prixVisAssemblageFenetre,
     null, 
     null, 
     null, 
     null, 
...
BigDecimal qteChassisOuvrant = (BigDecimal) subRow[24];
 ...
 prixChassisOuvrant,
    ...
    private JPanel createFenetrePanel() throws Exception {
        JPanel FenetrePanel = new JPanel(new BorderLayout());
        Color defaultColor = UIManager.getColor("ScrollBar.thumb");
        FenetrePanel.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 1, defaultColor));
      JPanel panelFenetreOptionContenaire = new JPanel(new BorderLayout());
        JPanel panelFenetreComboContenaire = new JPanel(new BorderLayout());
        JPanel panelFenetreSpinnerContenaire = new JPanel(new BorderLayout());
        panelFenetreOptionContenaire.add(createOptionsPanel(), BorderLayout.EAST);
        panelFenetreComboContenaire.add(createFenetreComboBoxPanel(), BorderLayout.NORTH);
        panelFenetreSpinnerContenaire.add(createSpinnerFenetrePanel(), BorderLayout.NORTH);
        FenetrePanel.add(panelFenetreComboContenaire, BorderLayout.WEST);
        FenetrePanel.add(panelFenetreSpinnerContenaire, BorderLayout.CENTER);
         FenetrePanel.add(panelFenetreOptionContenaire, BorderLayout.EAST);

        return FenetrePanel;
    }

    private JPanel createFenetreComboBoxPanel() throws Exception {
        ...
        DevisCreatePanel.fillComboBoxWithProducts(typeFenetreComboBox, "Fenetre");
            //String selectedprofileCouleur = (String)modelesProfileCouleursFenetreComboBox.getSelectedItem();
        DevisCreatePanel.populateModeleCouleurComboBox(modelesProfileCouleursFenetreComboBox);
       

        DevisCreatePanel.populateComboBox(vitrageFenetreComboBox, "vitrage", "type");
...
//            updateTypeVitrePorteComboBox(selectedVitrage);
            ...
            if (selectedVitrage != null && !selectedVitrage.isEmpty()) {
                try {
                    DevisCreatePanel.populateTypeVitreComboBox( typeVitreFenetreComboBox,selectedVitrage);
                } catch (Exception ex) {
                    Logger.getLogger(DevisCreatePanel.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        DevisCreatePanel.addField(firstCol, "Type de Fenêtre:", typeFenetreComboBox);
        DevisCreatePanel.addField(firstCol, "Modèle Profile:", modelesProfileCouleursFenetreComboBox);
        DevisCreatePanel.addField(firstCol, "Nombre de Ventaux:", nbVentauxFenetreSpinner);

        JPanel secondCol = new JPanel(new GridLayout(3, 1, 10, 10)); // 10 pixels horizontal gap
        DevisCreatePanel.addField(secondCol, "Vitrage:", vitrageFenetreComboBox);
        DevisCreatePanel.addField(secondCol, "Type de Vitre:", typeVitreFenetreComboBox);
...
//    panel.add(spacer, gbc); // Add spacer to create space between rows
    ...
    private void fillSpinnersAndQuantity(List<Object[]> subRows) {
        ...
        DevisCreatePanel.findSpinners(SpinnerContainerPanelFenetre, existingSpinners);
 ...
 int ventaux = (int) subRow[3];
              nbVentauxFenetreSpinner.setValue(ventaux ); 
            // Si nous avons des spinners existants, les réutiliser
            if (existingSpinnerIndex < existingSpinners.size()) {
                largeurFenetreSpinner = existingSpinners.get(existingSpinnerIndex);
                hauteurFenetreSpinner = existingSpinners.get(existingSpinnerIndex + 1);
                quantiteFenetreSpinner = existingSpinners.get(existingSpinnerIndex + 2);

               largeurFenetreSpinner.setValue(dim.largeur.doubleValue());
               hauteurFenetreSpinner.setValue(dim.hauteur.doubleValue());

                quantiteFenetreSpinner.setValue(quantite);
                ...
                JPanel dernierPanneauSousLigne = (JPanel) SpinnerContainerPanelFenetre.getComponent(SpinnerContainerPanelFenetre.getComponentCount() - 1);
                ...
                if (spinners.size() >= 3) {
                    largeurFenetreSpinner = spinners.get(0);
                    hauteurFenetreSpinner = spinners.get(1);
                    quantiteFenetreSpinner = spinners.get(2);
...
private void selectComboBoxItem(JComboBox<String> comboBox, String value) {
  ...
  private void fillFenetreComboBoxes(int selectedRow, List<Object[]> subRows) throws SQLException {
    ...
    if (produitId != null) {
        ...
        if (produit != null) {
            ...
            selectComboBoxItem(typeFenetreComboBox, nomProduit);
     ...
     Integer vitrageId = (Integer) subRow[4];
      if (vitrageId != null) {
        ...
        if ( vitre!= null) {
            ...
            selectComboBoxItem(vitrageFenetreComboBox,  nomStructure );
      ...
      if (typevitreId != -1) {
        ...
        if ( typevitre!= null) {
            ...
            selectComboBoxItem(typeVitreFenetreComboBox,  typeEpaisseur );
        ...
        if (chassifixe!= null&&chassisouvrant!= null) {
             ...
             if( profilechassisfixeId!=-1&&profilechassisouvrantId!=-1&& profilechassisouvrantId ==profilechassisfixeId ){
                   String CouleurModel =  chassifixe.getProfile().toString(); // Nom du produit à afficher
                     selectComboBoxItem(modelesProfileCouleursFenetreComboBox,  CouleurModel );
             }
    ...
    private void updateRemoveButtonsVisibilityFenetre() {
        ...
        for (JButton button : removeButtonsFenetre) {
            button.setVisible(removeButtonsFenetre.size() > 1);
        }
    }
