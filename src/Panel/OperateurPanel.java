...
import Form.AddOperateurForm;
import Form.AddOperateurForm;
import TableModel.OperateurTableModel;
 ...
 * @author brici_6ul2f65
 */
public class OperateurPanel extends javax.swing.JPanel {

    /**
     * Creates new form OperateurPanel
     */
private JPanel tablePanel;
    private JPanel titlePanel;
    private JLabel titleLabel;
    private ArrayList<ModelOperateur> Operateur;
    private OperateurTableModel OperateurTableModel;
   
    private TableRowSorter<OperateurTableModel> sorter;
    private ScrollPaneWin11 scroll;
    private JPanel leftPanel;
    private RoundedButton addButton;
    private CustomSearchField searchField;
    private RollOverTable OperateurTable;
     ...
     */
    public OperateurPanel() throws Exception {
     initComponents();
     init();
     initializeListeners();
     loadData();
   ...
   private void init(){
    ...
    JPanel OperateurTablePanel = new JPanel(new BorderLayout(0,5));
    OperateurTablePanel.setBorder(new EmptyBorder(15, 10, 15, 10));
    ...
    titlePanel.setPreferredSize(new Dimension(20, 35));
    titleLabel = new JLabel("Gestion de Sctock Operateur");
...
OperateurTableModel = new OperateurTableModel();

OperateurTable = new RollOverTable(OperateurTableModel);
// Utiliser le CustomTableCellRenderer

OperateurTable.setRowSorter(sorter);
OperateurTable.setFont(new Font("Dialog", Font.BOLD, 13));
OperateurTable.setRowHeight(30);
OperateurTable.setSelectionBackground(new Color(75, 110, 175));
configureActionColumnOperateur(OperateurTable, OperateurTableModel);
// Configurer l'apparence du header du tableau
JTableHeader header = OperateurTable.getTableHeader();
...
OperateurTable.setFillsViewportHeight(true);
OperateurTable.setShowGrid(true);
OperateurTable.setGridColor(Color.GRAY);
OperateurTable.setShowHorizontalLines(true);
OperateurTable.setShowVerticalLines(true);
  //  tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
    scroll = new ScrollPaneWin11();
    scroll.setViewportView(OperateurTable);
    //tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
    tablePanel.add(scroll, BorderLayout.CENTER);
    
  
    OperateurTablePanel.add(tablePanel, BorderLayout.CENTER);
     OperateurTablePanel.add(titlePanel, BorderLayout.NORTH);
  ...
  String[] columns = {"Choisir filtre","N° Operateur", "Modele Operateur", "Type","Prix Unitaire","Quantite","Fournisseur","Etat du stock"};
   ...
   leftPanel.add(searchChoiceCombo);
    ...
    OperateurPanel.add(OperateurTablePanel, BorderLayout.CENTER); // Add table panel in center
    OperateurPanel.add(actionPanel, BorderLayout.NORTH); // Add button panel above the table
    add( OperateurPanel, BorderLayout.CENTER);   
    ...
    private void configureActionColumnOperateur(JTable table, OperateurTableModel tableModel) {
        ...
        actionColumn.setCellEditor(new ActionCellEditor(new ActionTableEvent() {
            ...
            public void onEdit(int row) {
                try {
                    openEditOperateurForm(row); // Appeler la méthode pour ouvrir le dialogue d'édition
                } catch (SQLException ex) {
                    Logger.getLogger(OperateurPanel.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    if (text.isEmpty() || text.equals("Rechercher...")) {
        OperateurTable.setRowSorter(null);
        return;
    }

   sorter = new TableRowSorter<>(OperateurTableModel);
    ...
    OperateurTable.setRowSorter(sorter);
     ...
     public void openAddOperateurForm() throws SQLException {
         ModalDialog.showModal(this, new SimpleModalBorder(new AddOperateurForm(OperateurPanel.this), "Ajouter Operateur", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
             try {
                 loadData();
             } catch (Exception ex) {
                 Logger.getLogger(OperateurPanel.class.getName()).log(Level.SEVERE, null, ex);
        ...
        private void openEditOperateurForm(int row) throws SQLException {
              int modelRow = OperateurTable.convertRowIndexToModel(row);

        // Récupérer le client correspondant dans le modèle
        ModelOperateur Operateur = OperateurTableModel.getOperateurAt(modelRow);

        // Ouvrir la boîte de dialogue d'édition avec les informations du client
        AddOperateurForm dialog = new AddOperateurForm(OperateurPanel.this, Operateur);
       
          ModalDialog.showModal(this, new SimpleModalBorder(dialog, "Modifier Operateur", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
                  try {
                      loadData();
                  } catch (Exception ex) {
                      Logger.getLogger(SerrurePanel.class.getName()).log(Level.SEVERE, null, ex);
                  }
              OperateurTable.setRowSelectionInterval(row, row);
              OperateurTable.scrollRectToVisible(OperateurTable.getCellRect(row, 0, true)); 
            }));
            }
        private void deleteSelectedOperateur(int row) throws Exception {
    ModelOperateur Operateur = OperateurTableModel.getOperateurAt(row); // Remplacez par la méthode appropriée pour obtenir le Serrure à partir de la ligne
    ...
    if (response == JOptionPane.YES_OPTION) {
            ...
            DatabaseUtils.deleteOperateur(Operateur.getOperateurId()); // Appel à la méthode de suppression du Serrure
...
private void loadData() throws Exception  {
        Operateur = (ArrayList<ModelOperateur>) DatabaseUtils.getAllOperateurs(); // Récupérer les devis depuis la base de données
    OperateurTableModel.setOperateurList(Operateur); // Définir la liste de devis pour le modèle de table

        // Appliquer le renderer à la colonne ID
         OperateurTable.getColumnModel().getColumn(6).setCellRenderer(new StatusCellRenderer());
        OperateurTable.getColumnModel().getColumn(3).setCellRenderer(new RightAlignTableCellRenderer());
         for (int i : new int[]{0,4}) {
            OperateurTable.getColumnModel().getColumn(i).setCellRenderer(new CenterTableCellRenderer());
        }
