...
public class RoulettePanel extends javax.swing.JPanel {
    ...
    private ArrayList<ModelRoulette> roulette;
    private RouletteTableModel rouletteTableModel;
   
    private TableRowSorter<RouletteTableModel> sorter;
    private ScrollPaneWin11 scroll;
    private JPanel leftPanel;
    private RoundedButton addButton;
    private CustomSearchField searchField;
    private RollOverTable rouletteTable;
     ...
     */
    public RoulettePanel() throws Exception {
          // initComponents();
     init();
     initializeListeners();
     loadData() ;
   ...
   private void init(){
    ...
    titleLabel = new JLabel("Gestion de Sctock Roulette");
...
rouletteTableModel = new RouletteTableModel();

rouletteTable = new RollOverTable(rouletteTableModel);
// Utiliser le CustomTableCellRenderer

rouletteTable.setRowSorter(sorter);
rouletteTable.setFont(new Font("Dialog", Font.BOLD, 13));
rouletteTable.setRowHeight(30);
rouletteTable.setSelectionBackground(new Color(75, 110, 175));
configureActionColumnRoulette(rouletteTable, rouletteTableModel);
// Configurer l'apparence du header du tableau
JTableHeader header = rouletteTable.getTableHeader();
...
rouletteTable.setFillsViewportHeight(true);
rouletteTable.setShowGrid(true);
rouletteTable.setGridColor(Color.GRAY);
rouletteTable.setShowHorizontalLines(true);
rouletteTable.setShowVerticalLines(true);
  //  tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
    scroll = new ScrollPaneWin11();
    scroll.setViewportView(rouletteTable);
    //tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
   ...
   leftPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 1));
    ...
    String[] columns = {"Choisir filtre","N° Roulette", "Modele Profile ","Type","taille","Prix Unitaire","Quantite","Fournisseur","Etat du stock"};
   ...
   leftPanel.add(searchChoiceCombo);
    ...
    private void configureActionColumnRoulette(JTable table, RouletteTableModel tableModel) {
        ...
        actionColumn.setCellEditor(new ActionCellEditor(new ActionTableEvent() {
            ...
            public void onEdit(int row) {
                try {
                    openEditRouletteForm(row); // Appeler la méthode pour ouvrir le dialogue d'édition
                } catch (SQLException ex) {
                    Logger.getLogger(RoulettePanel.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    if (text.isEmpty() || text.equals("Rechercher...")) {
        rouletteTable.setRowSorter(null);
        return;
    }

   sorter = new TableRowSorter<>(rouletteTableModel);
    ...
    rouletteTable.setRowSorter(sorter);
     ...
     public void openAddRouletteForm() throws SQLException {
         ModalDialog.showModal(this, new SimpleModalBorder(new AddRouletteForm(RoulettePanel.this), "Ajouter Roulette", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
             try {
                 loadData();
             } catch (Exception ex) {
                 Logger.getLogger(RoulettePanel.class.getName()).log(Level.SEVERE, null, ex);
        ...
        private void openEditRouletteForm(int row) throws SQLException {
              int modelRow = rouletteTable.convertRowIndexToModel(row);

        // Récupérer le client correspondant dans le modèle
        ModelRoulette roulette = rouletteTableModel.getRouletteAt(modelRow);

        // Ouvrir la boîte de dialogue d'édition avec les informations du client
        AddRouletteForm dialog = new AddRouletteForm(RoulettePanel.this, roulette);
          ...
          ModalDialog.showModal(this, new SimpleModalBorder(dialog, "Modifier Serrure", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
              ...
              rouletteTable.setRowSelectionInterval(row, row);
              rouletteTable.scrollRectToVisible(rouletteTable.getCellRect(row, 0, true)); 
            }));
            }
        private void deleteSelectedRoulette(int row) throws Exception {
    ModelRoulette roulette = rouletteTableModel.getRouletteAt(row); // Remplacez par la méthode appropriée pour obtenir le Serrure à partir de la ligne
    ...
    if (response == JOptionPane.YES_OPTION) {
            ...
            DatabaseUtils.deleteRoulette(roulette.getRouletteId()); // Appel à la méthode de suppression du Serrure
   ...
   private void loadData() throws Exception  {
        roulette = (ArrayList<ModelRoulette>) DatabaseUtils.getAllRoulette(); // Récupérer les devis depuis la base de données
    rouletteTableModel.setRouletteList(roulette); // Définir la liste de devis pour le modèle de table

        // Appliquer le renderer à la colonne ID
     rouletteTable.getColumnModel().getColumn(7).setCellRenderer(new StatusCellRenderer());
        rouletteTable.getColumnModel().getColumn(4).setCellRenderer(new RightAlignTableCellRenderer());
         for (int i : new int[]{0,3,5}) {
            rouletteTable.getColumnModel().getColumn(i).setCellRenderer(new CenterTableCellRenderer());
        }
