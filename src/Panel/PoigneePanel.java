...
import Form.AddPoigneeForm;
import Models.ModelPoignee;
import java.sql.SQLException;
import TableModel.PoigneeTableModel;
 ...
 */
public class PoigneePanel extends javax.swing.JPanel {

    private JPanel tablePanel;
    private JPanel titlePanel;
    private JLabel titleLabel;
    private ArrayList<ModelPoignee> poignee;
    private PoigneeTableModel PoigneeTableModel;
   
    private TableRowSorter<PoigneeTableModel> sorter;
    private ScrollPaneWin11 scroll;
    private JPanel leftPanel;
    private RoundedButton addButton;
    private CustomSearchField searchField;
    private RollOverTable PoigneeTable;
     ...
     */
    public PoigneePanel() throws Exception {
           initComponents();
     init();
     initializeListeners();
     loadData();
   ...
   private void init(){
    ...
    JPanel poigneePanel = new JPanel(new BorderLayout());
    JPanel poigneeTablePanel = new JPanel(new BorderLayout(0,5));
    poigneeTablePanel.setBorder(new EmptyBorder(15, 10, 15, 10));
    tablePanel = new JPanel(new BorderLayout());
    
    titlePanel = new JPanel(new BorderLayout());
    titlePanel.setPreferredSize(new Dimension(20, 35));
    titleLabel = new JLabel("Gestion de Sctock Poignee");
    // Configurez le style du label si nécessaire
    titleLabel.setFont(new Font("SansSerif", Font.BOLD, 24));
    titleLabel.setHorizontalAlignment(SwingConstants.CENTER);
    titlePanel.add( titleLabel, BorderLayout.WEST);
    
     poignee = new ArrayList<>();
PoigneeTableModel = new PoigneeTableModel();

PoigneeTable = new RollOverTable(PoigneeTableModel);
// Utiliser le CustomTableCellRenderer

PoigneeTable.setRowSorter(sorter);
PoigneeTable.setFont(new Font("Dialog", Font.BOLD, 13));
PoigneeTable.setRowHeight(30);
PoigneeTable.setSelectionBackground(new Color(75, 110, 175));
configureActionColumnPoignee(PoigneeTable, PoigneeTableModel);
// Configurer l'apparence du header du tableau
JTableHeader header = PoigneeTable.getTableHeader();
...
PoigneeTable.setFillsViewportHeight(true);
PoigneeTable.setShowGrid(true);
PoigneeTable.setGridColor(Color.GRAY);
PoigneeTable.setShowHorizontalLines(true);
PoigneeTable.setShowVerticalLines(true);
  //  tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
    scroll = new ScrollPaneWin11();
    scroll.setViewportView(PoigneeTable);
    //tablePanelContenaire.add( titlePanel, BorderLayout.NORTH);
    tablePanel.add(scroll, BorderLayout.CENTER);
    
  
    poigneeTablePanel.add(tablePanel, BorderLayout.CENTER);
     poigneeTablePanel.add(titlePanel, BorderLayout.NORTH);
   ...
   String[] columns = {"Choisir filtre","N° Poignee", "Modele Profile",  "Type","Couleur","Prix Unitaire","Quantite","Fournisseur","Etat du stock"};
   ...
   leftPanel.add(searchChoiceCombo);
    ...
    poigneePanel.add(poigneeTablePanel, BorderLayout.CENTER); // Add table panel in center
    poigneePanel.add(actionPanel, BorderLayout.NORTH); // Add button panel above the table
    add( poigneePanel, BorderLayout.CENTER);  
    ...
    private void configureActionColumnPoignee(JTable table, PoigneeTableModel tableModel) {
        ...
        actionColumn.setCellEditor(new ActionCellEditor(new ActionTableEvent() {
            ...
            public void onEdit(int row) {
                try {
                    openEditPoigneeForm(row); // Appeler la méthode pour ouvrir le dialogue d'édition
                } catch (SQLException ex) {
                    Logger.getLogger(PoigneePanel.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    if (text.isEmpty() || text.equals("Rechercher...")) {
        PoigneeTable.setRowSorter(null);
        return;
    }

   sorter = new TableRowSorter<>(PoigneeTableModel);
    ...
    PoigneeTable.setRowSorter(sorter);
              ...
              private void openEditPoigneeForm(int row) throws SQLException {
              int modelRow = PoigneeTable.convertRowIndexToModel(row);

        // Récupérer le client correspondant dans le modèle
        ModelPoignee poignee = PoigneeTableModel.getPoigneeAt(modelRow);

        // Ouvrir la boîte de dialogue d'édition avec les informations du client
        AddPoigneeForm dialog = new AddPoigneeForm(PoigneePanel.this, poignee);
          ...
          ModalDialog.showModal(this, new SimpleModalBorder(dialog, "Modifier Client", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
              ...
              PoigneeTable.setRowSelectionInterval(row, row);
              PoigneeTable.scrollRectToVisible(PoigneeTable.getCellRect(row, 0, true)); 
            }));
            }

             private void deleteSelectedPoignee(int row) throws Exception {
    ModelPoignee poignee = PoigneeTableModel.getPoigneeAt(row); // Remplacez par la méthode appropriée pour obtenir le Poignee à partir de la ligne
    ...
    if (response == JOptionPane.YES_OPTION) {
            ...
            DatabaseUtils.deletePoignee(poignee.getPoigneeId()); // Appel à la méthode de suppression du Poignee

            loadData(); // Recharger les données depuis la base de données
        
    }
}
              public void openAddPoigneeForm() throws SQLException {
         ModalDialog.showModal(this, new SimpleModalBorder(new AddPoigneeForm(PoigneePanel.this), "Ajouter Poignee", SimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {
             try {
                 loadData();
             } catch (Exception ex) {
                 Logger.getLogger(PoigneePanel.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void loadData() throws Exception  {
        poignee = (ArrayList<ModelPoignee>) DatabaseUtils.getAllPoignee(); // Récupérer les devis depuis la base de données
   PoigneeTableModel.setPoigneeList(poignee); // Définir la liste de devis pour le modèle de table

        // Appliquer le renderer à la colonne ID
     PoigneeTable.getColumnModel().getColumn(7).setCellRenderer(new StatusCellRenderer());
       PoigneeTable.getColumnModel().getColumn(4).setCellRenderer(new RightAlignTableCellRenderer());
         for (int i : new int[]{0,5}) {
            PoigneeTable.getColumnModel().getColumn(i).setCellRenderer(new CenterTableCellRenderer());
        }
