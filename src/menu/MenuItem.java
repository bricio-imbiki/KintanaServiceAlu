    ...
    protected void setSelectedIndex(int index) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (com instanceof JButton) {
                ((AbstractButton) com).setSelected(i == index);
                if (i == index) {
                    selected = true;
                }
            }
        }
        ((AbstractButton) getComponent(0)).setSelected(selected);
    ...
    public void paint(Graphics g) {
        ...
        if (menus.length > 1) {
            ...
            if (menu.isMenuFull()) {
                ...
                int ax = ltr ? (getWidth()+3 - arrowWidth - UIScale.scale(3)) : UIScale.scale(3);
                int ay = (smenuItemHeight - arrowHeight) / 2;
                Path2D p = new Path2D.Double();
                if (ltr) {
                    p.moveTo(0, 0);
